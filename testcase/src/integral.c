...
FLOAT computeT(int n, FLOAT a, FLOAT b, FLOAT (*fun)(FLOAT)) {
	...
	*/
	...
	for(k = 1; k < n; k ++) {
		...
		switch(k){
				case 1: {
								//nemu_assert(Fabs(a + F_mul_int(h,k) - f2F(-0.8)) < f2F(1e-4i));
								FLOAT x = a + F_mul_int(h, k);
								nemu_assert(F_mul_F(x, x) - f2F(0.64) < f2F(1e-4));
								x = F_mul_int(F_mul_F(x, x), 25);
								nemu_assert(x - f2F(16) < f2F(1e-4));
								x = x + int2F(1);
								nemu_assert(x - f2F(17) < f2F(1e-4));
								x = F_div_F(int2F(1), x);
								nemu_assert(x - f2F(0.058824) < f2F(1e-4));
