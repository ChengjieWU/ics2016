...
make_helper(concat(jcc_l_,SUFFIX)) {
	...
	if (cpu.SF != cpu.OF) cpu.eip = cur_addr - (1 + DATA_BYTE) - (DATA_BYTE != 1);
	return DATA_BYTE + 1;	
}
...
make_helper(concat(jcc_le_,SUFFIX)) {
	...
	if (cpu.ZF == 1 || cpu.SF != cpu.OF) cpu.eip = cur_addr - (1 + DATA_BYTE) - (DATA_BYTE != 1);
	return DATA_BYTE + 1;	
}
...
make_helper(concat(jcc_ne_,SUFFIX)) {
	...
	if (cpu.ZF == 0) cpu.eip = cur_addr - (1 + DATA_BYTE);
	return DATA_BYTE + 1;	
}
...
make_helper(concat(jcc_no_,SUFFIX)) {
	...
	if (cpu.OF == 0) cpu.eip = cur_addr - (1 + DATA_BYTE);
	return DATA_BYTE + 1;	
}
...
make_helper(concat(jcc_np_,SUFFIX)) {
	...
	if (cpu.PF == 0) cpu.eip = cur_addr - (1 + DATA_BYTE);
	return DATA_BYTE + 1;	
}
...
make_helper(concat(jcc_ns_,SUFFIX)) {
	...
	if (cpu.SF == 0) cpu.eip = cur_addr - (1 + DATA_BYTE);
	return DATA_BYTE + 1;	
}
